...
public class NetheriteBeaconBlockEntity extends BeaconBlockEntity implements NamedScreenHandlerFactory, Tickable {
    ...
    public void tick() {
        ...
        NetheriteBeaconBlockEntity.BeamSegment beamSegment = this.field_19178.isEmpty() ? null : (NetheriteBeaconBlockEntity.BeamSegment)this.field_19178.get(this.field_19178.size() - 1);
            ...
            if (block instanceof Stainable) {
                ...
                if (this.field_19178.size() <= 1) {
                    ...
                    if (Arrays.equals(fs, beamSegment.color)) {
                        ...
                        beamSegment = new NetheriteBeaconBlockEntity.BeamSegment(new float[]{(beamSegment.color[0] + fs[0]) / 2.0F, (beamSegment.color[1] + fs[1]) / 2.0F, (beamSegment.color[2] + fs[2]) / 2.0F});
                        this.field_19178.add(beamSegment);
                    }
                }
            } else {
